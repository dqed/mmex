...
class Game_change_goldModel extends BaseplayinfoModel {
	...
	public function getplaysInfo($conds){
		...
		foreach ($conds as $key => $value) {
			// $map["DATE_FORMAT(change_time,'%Y-%m-%d')"]=$value['time'];
			$data=$this->query("select DATE_FORMAT(change_time,'%Y-%m-%d') days,count(user_id),count(DISTINCT user_id) from table_game_change_gold where user_id>0 and DATE_FORMAT(change_time,'%Y-%m-%d') ='".$value['time']."'");
	...
	public function exceldata($cond){
            ...
            foreach ($pp as $kkk => $vvv) {  
               ...
               exportToExcel('对局记录_'.time().$vvv.'.csv',$exl11,$exl[$kkk]);  
            }  
            exit();  
    	}
}